...
class Visualizer:
    def __init__(self, app, x, y, width, height, ctx, offscreen_fbo, lighting_fbo, video, normalized_piano_y):
        ...
        self.piano = Piano(self.ctx, self.rect, normalized_piano_y)
    ...
    def render(self, frame_time, current_time, paused=False):
 
    
        self.piano.draw(self.left_cutoff, self.right_cutoff)
        
        if current_time > -self.video.start_time:
            self.video.render(frame_time, current_time)
        self.particle_system.update(frame_time)
        self.saber.render()
        self.background.render(frame_time if not paused else 0)
...
class PianoKey:
    def __init__(self, ctx, index, visualizer_rect, normalized_piano_y):
        ...
        if white_key_indices.__contains__(self.index):
            ...
            self.height *= 0.6
        self.y = self.visualizer_rect.height * normalized_piano_y + self.visualizer_rect.y
...
class Piano:
    ...
    def draw(self, left_cutoff, right_cutoff):
        for key in keys:
            key.draw(left_cutoff, right_cutoff)

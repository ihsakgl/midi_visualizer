...
uniform vec4 noteColor1;
uniform vec4 noteColor2;
...
void main() {
    ...
    vec4 noteColor;
    noteColor.a = noteColor1.a;

    if (fragCoord.x < leftCutoff) {
        noteColor.rgb = noteColor1.rgb;
    } else if (fragCoord.x > rightCutoff) {
        noteColor.rgb = noteColor2.rgb;
    } else {
        float t = clamp((fragCoord.x - leftCutoff) / (rightCutoff - leftCutoff), 0.0, 1.0);
        noteColor.rgb = mix(noteColor1.rgb, noteColor2.rgb, t);
    }
